    ...
    <section id="overview" className="py-20 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-slate-100 mb-3">Kollab 是什么？</h2>
          <p className="text-slate-400 max-w-xl mx-auto">不只是对话框。一个带工具、带集成、能干活的 AI 工作台。</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map((f) => (
            <div key={f.title} className="p-5 bg-[var(--surface)] border border-[var(--border)] rounded-xl card-hover">
              ...
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">{f.title}</h3>
              <p className="text-xs text-slate-400 leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
        <div className="mt-10 p-6 bg-[var(--surface)] border border-[var(--border)] rounded-xl">
          <p className="text-xs text-slate-400 mb-4 text-center">工作原理</p>
          <div className="flex items-center justify-center gap-3 flex-wrap text-xs">
            {["你的消息", "→", "Claude 理解", "→", "调用技能/工具", "→", "执行任务", "→", "返回结果"].map((item, i) => (
              <span key={i} className={item === "→" ? "text-slate-500" : "px-3 py-1.5 bg-[var(--surface2)] border border-[var(--border)] rounded-md text-slate-400"}>{item}</span>
            ))}
          </div>
        </div>
      </div>
