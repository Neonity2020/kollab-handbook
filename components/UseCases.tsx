...
export default function UseCases() {
  ...
  return (
    <section id="use-cases" className="py-20 px-4 bg-[var(--bg)]">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-10">
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-slate-100 mb-3">核心用例</h2>
          <p className="text-slate-400 max-w-xl mx-auto">8 个真实场景，每个都附有可直接使用的提示词模板。</p>
        </div>
        <div className="flex items-center gap-2 mb-8 flex-wrap">
          {Object.entries(categoryLabels).map(([key, label]) => (
            <button key={key} onClick={() => setActiveCategory(key)} className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${activeCategory === key ? "bg-blue-600 text-slate-900 dark:text-slate-900 dark:text-white" : "bg-[var(--bg)] border border-[var(--border)] text-slate-400 hover:border-blue-500/40 hover:text-slate-300"}`}>{label}</button>
          ))}
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((uc) => {
            ...
            return (
              <div key={uc.id} className="bg-[var(--bg)] border border-[var(--border)] rounded-xl overflow-hidden card-hover">
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-200">{uc.title}</h3>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {uc.tags.map((tag) => <span key={tag} className="px-2 py-0.5 bg-[var(--surface)] border border-[var(--border)] rounded text-xs text-slate-500">{tag}</span>)}
                  </div>
                ...
                {isExpanded && (
                  <div className="border-t border-[var(--border)] p-5 space-y-4">
                    ...
                    <div>
                      ...
                      <pre className="bg-[var(--bg)] border border-[var(--border)] rounded-lg p-3 text-xs text-slate-400 whitespace-pre-wrap leading-relaxed overflow-x-auto">{uc.promptTemplate}</pre>
                    </div>
                  </div>
                )}
              </div>
