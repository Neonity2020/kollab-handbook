    ...
    <>
      ...
      <main className="min-h-screen bg-[var(--bg)] pt-24 pb-16 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="mb-12">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-500 dark:text-blue-400 text-xs mb-4">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" />
              每日更新
            </div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-3">技术前沿追踪</h1>
          ...
          <div className="space-y-6">
            {blogPosts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}/`}
                className="group block bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 hover:border-blue-500/40 hover:shadow-md transition-all duration-200"
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {post.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 text-xs rounded-md bg-blue-500/10 text-blue-500 dark:text-blue-400 border border-blue-500/20"
                    >
                      {tag}
                    </span>
                  ))}
